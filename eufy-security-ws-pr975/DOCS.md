...
Unofficial temporary build. See the public project documentation and rollback
instructions at:

https://github.com/jackpott-tech/eufy-security-ws-pr975-ha-addon

This local add-on bundles `eufy-security-ws 3.1.0-pr975.1` and
`eufy-security-client 4.1.0-pr975.1`. It is intentionally configured for
manual start and port `3001`, so the stable add-on remains installed for a
fast rollback. Do not run both instances longer than needed with the same
Eufy account.

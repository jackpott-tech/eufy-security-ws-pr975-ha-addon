...
import { EufySecurity, LogLevel, } from "eufy-security-client";
...
(async () => {
    ...
    const driver = await EufySecurity.initialize(config, logger.getSubLogger({ name: "eufy-security-client" }));
    let server;
    try {
        server = new EufySecurityServer(driver, {
            port: args.port,
            host: args.host,
            logger: logger,
        });

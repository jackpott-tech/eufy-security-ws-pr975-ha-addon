...
const commands = Object.values(DriverCommand)
    .concat(Object.values(DeviceCommand))
    .concat(Object.values(StationCommand))
    .concat(["quit", "exit"]);
const devicePropertiesMetadata = {};
const stationPropertiesMetadata = {};
const emitter = new EventEmitter();
initializeInspectStyles();
let silendCommandTimeout;
let closing = false;
...
const parsePropertyValue = (property, value, serialNumber, propertiesMetadata) => {
    try {
        if (propertiesMetadata[serialNumber] !== undefined &&
...
const cmd = async (args, silent = false, internal = false) => {
    switch (args[0]) {
        case "help":
        ...
        case DeviceCommand.getPropertiesMetadata:
            if (args.length === 2) {
                socket.send(JSON.stringify({
                    messageId: internal
                        ? `internal_${DeviceCommand.getPropertiesMetadata}`
        ...
        case DeviceCommand.snooze:
            if ((args.length === 3 && isNumber(args[2])) ||
                ...
                socket.send(JSON.stringify({
                    ...
                    snoozeChime: args.length >= 4 && args[3].toLowerCase() === "true"
                        ? true
                        : false,
                    snoozeMotion: args.length >= 5 && args[4].toLowerCase() === "true"
                        ? true
                        : false,
                    snoozeHomebase: args.length === 6 && args[5].toLowerCase() === "true"
                        ? true
                        : false,
                }));
            }
        ...
        case DeviceCommand.updateUserSchedule:
            if (args.length === 4) {
                const command = {
                    ...
                    serialNumber: args[1],
                    username: args[2],
                    schedule: args[3], //TODO: Correct type casting etc.
                };
                socket.send(JSON.stringify(command));
            }
            else {
        ...
        case StationCommand.getPropertiesMetadata:
            if (args.length === 2) {
                socket.send(JSON.stringify({
                    messageId: internal
                        ? `internal_${StationCommand.getPropertiesMetadata}`
...
socket.on("message", (data) => {
    ...
    if (msg.type === "event") {
        ...
        if (event.event.source === "server" && event.event.event === "shutdown") {
            ...
            if (source === "device") {
                ...
                const stationEvent = event.event;
                if (stationEvent.source === source &&
        ...
        switch (resultMsg.messageId) {
            case "start-listening-result": {
                if (options.command === undefined) {
                    ...
                    cmd([options.command].concat(options.arguments !== undefined
                        ? options.arguments
            ...
            case convertCamelCaseToSnakeCase(options.command?.split(".")[1]):
                if (resultMsg.success) {
                    ...
                    if (msgSuccess.result &&
                        msgSuccess.result.async === undefined) {
    ...
    if ((msg.type === "result" && !msg.messageId.startsWith("internal_")) ||
        msg.type !== "result") {
        if (options.command === undefined) {
            if (options.verbose) {
                if (rl)

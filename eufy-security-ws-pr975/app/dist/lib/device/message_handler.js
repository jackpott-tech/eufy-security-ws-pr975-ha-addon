...
export class DeviceMessageHandler {
    static streamingDevices = {};
    static downloadingDevices = {};
    static talkbackingDevices = {};
    static talkbackStream;
    static getStreamingDevices(stationSN) {
        if (DeviceMessageHandler.streamingDevices[stationSN] !== undefined)
            return DeviceMessageHandler.streamingDevices[stationSN];
        return [];
    }
    static removeStreamingDevice(stationSN, client) {
        if (DeviceMessageHandler.streamingDevices[stationSN] !== undefined)
            DeviceMessageHandler.streamingDevices[stationSN] =
                DeviceMessageHandler.streamingDevices[stationSN].filter((cl) => cl !== client);
    }
    static addStreamingDevice(stationSN, client) {
        if (DeviceMessageHandler.streamingDevices[stationSN] !== undefined &&
    ...
    static getDownloadingDevices(stationSN) {
        if (DeviceMessageHandler.downloadingDevices[stationSN] !== undefined)
            return DeviceMessageHandler.downloadingDevices[stationSN];
        return [];
    }
    static removeDownloadingDevice(stationSN, client) {
        if (DeviceMessageHandler.downloadingDevices[stationSN] !== undefined)
            DeviceMessageHandler.downloadingDevices[stationSN] =
                DeviceMessageHandler.downloadingDevices[stationSN].filter((cl) => cl !== client);
    }
    static addDownloadingDevice(stationSN, client) {
        if (DeviceMessageHandler.downloadingDevices[stationSN] !== undefined &&
    ...
    static getTalkbackingDevices(stationSN) {
        if (DeviceMessageHandler.talkbackingDevices[stationSN] !== undefined)
            return DeviceMessageHandler.talkbackingDevices[stationSN];
        return [];
    }
    static removeTalkbackingDevice(stationSN, client) {
        if (DeviceMessageHandler.talkbackingDevices[stationSN] !== undefined)
            DeviceMessageHandler.talkbackingDevices[stationSN] =
                DeviceMessageHandler.talkbackingDevices[stationSN].filter((cl) => cl !== client);
    }
    static addTalkbackingDevice(stationSN, client) {
        if (DeviceMessageHandler.talkbackingDevices[stationSN] !== undefined &&
        ...
        switch (command) {
            case DeviceCommand.setStatusLed:
                if (client.schemaVersion <= 12) {
                    station.setStatusLed(device, message.value);
                    return {};
                }
                else {
            ...
            case DeviceCommand.setMotionDetection:
                if (client.schemaVersion <= 12) {
                    station.setMotionDetection(device, message.value);
                    return {};
                }
                else {
                    throw new UnknownCommandError(command);
            ...
            case DeviceCommand.setSoundDetection:
                if (client.schemaVersion <= 12) {
                    station.setSoundDetection(device, message.value);
                    return {};
                }
                else {
                    throw new UnknownCommandError(command);
            ...
            case DeviceCommand.setPetDetection:
                if (client.schemaVersion <= 12) {
                    station.setPetDetection(device, message.value);
                    return {};
                }
                else {
                    throw new UnknownCommandError(command);
            ...
            case DeviceCommand.setRTSPStream:
                if (client.schemaVersion <= 12) {
                    station.setRTSPStream(device, message.value);
                    return {};
                }
                else {
                    throw new UnknownCommandError(command);
            ...
            case DeviceCommand.setAntiTheftDetection:
                if (client.schemaVersion <= 12) {
                    station.setAntiTheftDetection(device, message.value);
                    return {};
                }
                else {
                    throw new UnknownCommandError(command);
            ...
            case DeviceCommand.setWatermark:
                if (client.schemaVersion <= 12) {
                    station.setWatermark(device, message.value);
                    return {};
                }
                else {
                    throw new UnknownCommandError(command);
            ...
            case DeviceCommand.enableDevice:
                if (client.schemaVersion <= 12) {
                    station.enableDevice(device, message.value);
                    return {};
                }
                else {
                    throw new UnknownCommandError(command);
            ...
            case DeviceCommand.lockDevice:
                if (client.schemaVersion <= 12) {
                    station.lockDevice(device, message.value);
                    return {};
                }
                else {
                    throw new UnknownCommandError(command);
            ...
            case DeviceCommand.getProperties: {
                ...
                if (client.schemaVersion <= 3) {
                    ...
                    return {
                        serialNumber: device.getSerial(),
                        properties: dumpDeviceProperties(device, client.schemaVersion),
                    };
                }
            }
            ...
            case DeviceCommand.stopLivestream:
                if (client.schemaVersion >= 2) {
                    ...
                    if (DeviceMessageHandler.streamingDevices[station.getSerial()] !==
                        undefined &&
            ...
            case DeviceCommand.triggerAlarm:
                if (client.schemaVersion >= 3) {
                    station.triggerDeviceAlarmSound(device, message.seconds);
                    return client.schemaVersion >= 13 ? { async: true } : {};
                }
                else {
                    throw new UnknownCommandError(command);
            ...
            case DeviceCommand.quickResponse:
                if (client.schemaVersion >= 3) {
                    station.quickResponse(device, message.voiceId);
                    return client.schemaVersion >= 13 ? { async: true } : {};
                }
                else {
                    throw new UnknownCommandError(command);
            ...
            case DeviceCommand.startDownload:
                if (client.schemaVersion >= 3) {
                    if (!station.isDownloading(device)) {
                        await station
                            .startDownload(device, message.path, message.cipherId)
                            .catch((error) => {
                            throw error;
                        });
            ...
            case DeviceCommand.cancelDownload:
                if (client.schemaVersion >= 3) {
                    ...
                    if (DeviceMessageHandler.downloadingDevices[station.getSerial()] !==
                        undefined &&
            ...
            case DeviceCommand.hasProperty: {
                if (client.schemaVersion >= 3) {
                    const result = device.hasProperty(message.propertyName);
                    if (client.schemaVersion === 3) {
            ...
            case DeviceCommand.hasCommand: {
                if (client.schemaVersion >= 3) {
                    const result = device.hasCommand(message.commandName);
                    if (client.schemaVersion === 3) {
                        return { exists: result };
                    }
                    else if (client.schemaVersion >= 4) {
            ...
            case DeviceCommand.getCommands: {
                if (client.schemaVersion >= 3) {
                    ...
                    if (client.schemaVersion === 3) {
                        ...
                        return {
                            serialNumber: device.getSerial(),
                            commands: commands, //TODO: Convert to correct type
                        };
                    }
                }
                else {
            ...
            case DeviceCommand.stopTalkback:
                if (client.schemaVersion >= 13) {
                    ...
                    if (DeviceMessageHandler.talkbackingDevices[station.getSerial()] !==
                        undefined &&
            ...
            case DeviceCommand.talkbackAudioData:
                if (client.schemaVersion >= 13) {
                    ...
                    if (DeviceMessageHandler.talkbackingDevices[station.getSerial()] !==
                        undefined &&
                        DeviceMessageHandler.talkbackingDevices[station.getSerial()].includes(client)) {
                        const audioDataBuffer = Buffer.from(message.buffer);
            ...
            case DeviceCommand.snooze:
                if (client.schemaVersion >= 13) {
                    station.snooze(device, {
                        snooze_time: message.snoozeTime,
                        snooze_chime: message.snoozeChime,
                        snooze_motion: message
                            .snoozeMotion,
                        snooze_homebase: message
                            .snoozeHomebase,
                    });
                    return { async: true };
                }
            ...
            case DeviceCommand.updateUser:
                if (client.schemaVersion >= 13) {
                    const addUser = message;
                    await driver
                        .updateUser(device.getSerial(), addUser.username, addUser.newUsername)
                        .catch((error) => {
                        throw error;
                    });
                    return { async: true };
            ...
            case DeviceCommand.updateUserPasscode:
                if (client.schemaVersion >= 13) {
                    const addUser = message;
                    await driver
                        .updateUserPasscode(device.getSerial(), addUser.username, addUser.passcode)
                        .catch((error) => {
                        throw error;
                    });
                    return { async: true };
            ...
            case DeviceCommand.updateUserSchedule:
                if (client.schemaVersion >= 13) {
                    const addUser = message;
                    await driver
                        .updateUserSchedule(device.getSerial(), addUser.username, addUser.schedule)
                        .catch((error) => {
                        throw error;
                    });
                    return { async: true };
            ...
            case DeviceCommand.savePresetPosition:
                if (client.schemaVersion >= 21) {
                    const savePresetPosition = message;
            ...
            case DeviceCommand.deletePresetPosition:
                if (client.schemaVersion >= 21) {
                    const deletePresetPosition = message;
